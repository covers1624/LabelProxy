...
public class NginxConfigGenerator {
    ...
    private final IndentPrintWriter pw = new IndentPrintWriter(new PrintWriter(sw, true));

    public NginxConfigGenerator(Config config, LetsEncryptService letsEncrypt, NginxService.NginxHost host) {
        this.config = config;
    ...
    private void emitBraced(String key, Runnable action) {
        pw.println(key + " {");
        pw.pushIndent();
        action.run();
        pw.popIndent();
    ...
    private static String addStart(String add, String str) {
